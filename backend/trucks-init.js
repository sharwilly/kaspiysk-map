...
function normalize(raw) {
    ...
    return { vehicle_id: vehicle, recorded_at: time.toISOString(), latitude: lat, longitude: lng,
        route: String(raw.lineid ?? raw.route ?? raw.line ?? '').trim() || null,
        location: String(raw.location ?? '').trim() || null, speed: num(raw.speed ?? raw.SpeedValue) };
}
function recordsFrom(payload) {
    if (Array.isArray(payload)) return payload;
    for (const key of ['data', 'records', 'result', 'results']) if (Array.isArray(payload?.[key])) return payload[key];
    return [];
}
async function ensureTable() {
    if (tableReady) return;
    await pool.query(`CREATE TABLE IF NOT EXISTS truck_gps_points (
        id BIGSERIAL PRIMARY KEY, vehicle_id TEXT NOT NULL, recorded_at TIMESTAMPTZ NOT NULL,
        latitude DOUBLE PRECISION NOT NULL, longitude DOUBLE PRECISION NOT NULL, route TEXT,
        location TEXT, source TEXT NOT NULL DEFAULT 'new_taipei_demo', speed DOUBLE PRECISION,
...
async function pollSource() {
    const response = await fetch(SOURCE_URL, { headers: { Accept: 'application/json', 'User-Agent': 'OpenKaspiysk-Demo/1.0' }, signal: AbortSignal.timeout(8000) });
    if (!response.ok) throw new Error(`GPS source HTTP ${response.status}`);
    const payload = await response.json(); const latest = new Map();
    for (const raw of recordsFrom(payload)) {
        const point = normalize(raw); if (!point) continue;
    ...
    for (const point of latest.values()) {
        await pool.query(`INSERT INTO truck_gps_points (vehicle_id, recorded_at, latitude, longitude, route, location, source, speed)
            SELECT $1,$2,$3,$4,$5,$6,'new_taipei_demo',$7 WHERE NOT EXISTS
...
async function latestTrucks() {
    const result = await pool.query(`SELECT DISTINCT ON (vehicle_id) vehicle_id AS id, vehicle_id AS vehicle,
        recorded_at AS timestamp, latitude AS lat, longitude AS lng, route, location, speed,
        EXTRACT(EPOCH FROM (NOW() - recorded_at))/60 AS age_minutes
        FROM truck_gps_points ORDER BY vehicle_id, recorded_at DESC`);
    return result.rows.sort((a,b) => new Date(b.timestamp) - new Date(a.timestamp)).slice(0, MAX_TRUCKS)
        .map(r => ({...r, fresh: Number(r.age_minutes) <= STALE_MINUTES, ageMinutes: Math.round(Number(r.age_minutes)*10)/10}));
}
async function history(vehicleId, date) {
    const result = await pool.query(`SELECT recorded_at AS timestamp, latitude AS lat, longitude AS lng, route, location, speed
        FROM truck_gps_points WHERE vehicle_id=$1 AND recorded_at >= $2::date AND recorded_at < ($2::date + INTERVAL '1 day')
        ORDER BY recorded_at ASC`, [vehicleId, date]);
    return result.rows;
}
...
function registerRoutes(app, originalGet) {
    ...
    originalGet.call(app, '/trucks', async (req, res) => {
        try {
            ...
            res.json({ trucks, count: trucks.length, source: 'Новый Тайбэй (демо)', sourceError,
