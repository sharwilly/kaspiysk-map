...
document.getElementById("myLocation")?.addEventListener("click",()=>{if(!navigator.geolocation)return alert("Геолокация не поддерживается вашим браузером");navigator.geolocation.getCurrentPosition(async position=>{const{latitude,longitude}=position.coords;if(cityBoundary&&!turf.booleanPointInPolygon(turf.point([longitude,latitude]),cityBoundary))return alert("Ваше местоположение находится за пределами Каспийска");selectedLocation={latitude,longitude};selectedAddress="Получение адреса...";if(tempMarker)map.removeLayer(tempMarker);tempMarker=L.marker([latitude,longitude]).addTo(map).bindTooltip("📍 Получение адреса...",{permanent:true,direction:"top",offset:[0,-10]}).openTooltip();map.setView([latitude,longitude],17);try{const response=await fetch(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&addressdetails=1&lat=${latitude}&lon=${longitude}`,{headers:{"User-Agent":"KaspiyskMap/1.0"}});if(response.ok)selectedAddress=shortAddress(await response.json());}catch(error){selectedAddress="Адрес не определён";}tempMarker.unbindTooltip().bindTooltip("📍 "+selectedAddress,{permanent:true,direction:"top",offset:[0,-10]}).openTooltip();document.getElementById("addressResults").innerHTML="Выбрано: 📍 "+escapeHtml(selectedAddress);},()=>alert("Не удалось определить местоположение"));});

function renderPhotoPreview(){const container=document.getElementById("photoPreview");if(!container)return;container.innerHTML=selectedPhotos.map((file,index)=>`<div class="photo-preview-item"><img src="${URL.createObjectURL(file)}" alt="Фото"><button type="button" data-index="${index}">✕</button></div>`).join("");container.querySelectorAll("button").forEach(button=>button.addEventListener("click",()=>{selectedPhotos.splice(Number(button.dataset.index),1);renderPhotoPreview();}));}
document.getElementById("photos")?.addEventListener("change",event=>{const files=Array.from(event.target.files||[]);selectedPhotos=[...selectedPhotos,...files].slice(0,3);renderPhotoPreview();});
function showSuccessMessage(id){const notice=document.createElement("div");notice.className="success-toast";notice.textContent=`Обращение №${id} отправлено`;document.body.appendChild(notice);setTimeout(()=>notice.remove(),4000);}

(async()=>{await loadCityBoundary();await loadProblemsOnMap();await loadOutagesOnMap();await applyQueryParameters();})();
